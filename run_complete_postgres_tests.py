...
def check_docker():
    ...
    stdout, stderr, code = run_command("docker info", "Docker daemon check", capture_output=True, check=False)
    if code != 0:
...
def setup_postgres_container():
    ...
    for attempt in range(10):
        if run_command("docker exec marketpipe-postgres pg_isready -U marketpipe", "PostgreSQL ready check", capture_output=True, check=False):
...
async def test_basic_connection():
    ...
    try:
        ...
        # Test GIN index
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_test_jsonb_gin ON test_jsonb USING GIN (data)")
...
def test_alembic_migrations():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            expected_tables = [
                'alembic_version', 'checkpoints', 'ingestion_jobs',
                'metrics', 'ohlcv_bars', 'symbol_bars_aggregates'
            ]
...
def show_summary():
    ...
    print("  Pytest tests: TEST_POSTGRES=1 POSTGRES_TEST_URL=postgresql://marketpipe:password@localhost:5433/marketpipe pytest tests/test_migrations.py::TestPostgresMigrations -v")
