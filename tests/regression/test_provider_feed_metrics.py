...
class TestProviderFeedLabelsFixed:
    ...
    def test_record_metric_forwards_to_prometheus(self):
        ...
        record_metric(
            "test_request",
            1.0,
            provider="test_alpaca_unique",
            feed="test_iex_unique",
            source="test_unique",
        )
        ...
        request_samples = [
            s
            for s in REQUESTS.collect()[0].samples
            if s.labels.get("provider") == "test_alpaca_unique"
        ]
        ...
        record_metric(
            "test_error_failed",
            1.0,
            provider="test_polygon_unique",
            feed="test_sip_unique",
            source="test_validation_unique",
        )

        error_samples = [
            s
            for s in ERRORS.collect()[0].samples
            if s.labels.get("provider") == "test_polygon_unique"
        ]
        ...
        record_metric(
            ...
            1.5,
            provider="test_alpaca_latency",
            feed="test_iex_latency",
            source="test_ingestion_unique",
        )

        latency_samples = [
            s
            for s in LATENCY.collect()[0].samples
            if s.labels.get("provider") == "test_alpaca_latency"
        ]
    ...
    def test_alpaca_client_uses_new_labels(self):
        ...
        with patch("httpx.get") as mock_get:
            ...
            assert (
                len(alpaca_requests) > 0
            ), f"Expected alpaca metrics not found. Available samples: {[(s.labels, s.value) for s in request_samples]}"
    ...
    def test_legacy_metrics_backward_compatibility(self):
        ...
        record_metric(
            "test_request",
            1.0,
            provider="legacy_test_alpaca",
            feed="legacy_test_iex",
            source="legacy_test_source",
        )
        record_metric(
            "test_error_timeout",
            1.0,
        ...
        legacy_requests = [
            s
            for s in LEGACY_REQUESTS.collect()[0].samples
            if s.labels.get("source") == "legacy_test_source"
        ]
        ...
        legacy_errors = [
            s
            for s in LEGACY_ERRORS.collect()[0].samples
            if s.labels.get("source") == "legacy_test_source"
        ]
        ...
        legacy_latency = [
            s
            for s in LEGACY_LATENCY.collect()[0].samples
            if s.labels.get("source") == "legacy_test_source"
        ]
