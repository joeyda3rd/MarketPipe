...
from tests.fakes.adapters import FakeHttpClient, FakeAsyncHttpClient
...
class TestAlpacaClientWithFakes:
    ...
    def test_handles_paginated_response(self, alpaca_config, auth_strategy):
        ...
        http_client.configure_response(
            ...
            status=200,
            body={
                "bars": [{"S": "AAPL", "t": "2023-01-02T09:30:00Z", "o": 100, "h": 101, "l": 99, "c": 100.5, "v": 1000}],
        ...
        http_client.configure_response(
            ...
            status=200,
            body={
                "bars": [{"S": "AAPL", "t": "2023-01-02T09:31:00Z", "o": 100.5, "h": 101.5, "l": 99.5, "c": 101.0, "v": 1500}]
            }
        )
        ...
        with unittest.mock.patch.object(client, '_request', side_effect=self._simulate_request_with_fake(http_client)):
        ...
        assert all(row["schema_version"] == 1 for row in rows), "All bars should have schema version"
    ...
    async def test_async_client_behavior(self, alpaca_config, auth_strategy):
        ...
        with unittest.mock.patch.object(client, '_async_request', side_effect=self._simulate_async_request_with_fake(async_http_client)):
    ...
    def test_error_handling_scenarios(self, alpaca_config, auth_strategy):
        ...
        test_cases = [
            ...
            {
                "name": "server_error",
                "error": httpx.HTTPStatusError("Server error", request=None, response=unittest.mock.Mock(status_code=500)),
