...
class TestRepositoryFactoryFixes:
    ...
    @pytest.mark.parametrize("database_url,expected_type", [
        ...
        ("mysql://user:pass@localhost/db", "SqliteIngestionJobRepository"),  # Falls back to SQLite
    ...
    def test_database_url_detection(self, database_url, expected_type):
        ...
        with patch.dict('os.environ', {'DATABASE_URL': database_url} if database_url else {}, clear=True):
            with patch('marketpipe.ingestion.infrastructure.repository_factory.PostgresIngestionJobRepository') as mock_postgres:
                with patch('marketpipe.ingestion.infrastructure.repository_factory.SqliteIngestionJobRepository') as mock_sqlite:
...
class TestSimpleJobAdapterFixes:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("input_status,expected_normalized", [
        ("pending", "pending"),
        ("PENDING", "pending"),
        ("Pending", "pending"),
        ("RUNNING", "running"),
    ...
    async def test_status_normalization_in_upsert(self, simple_adapter, input_status, expected_normalized):
    ...
    async def test_status_normalization_in_mark_done(self, simple_adapter, input_status, expected_normalized):
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("input_status", [
    ...
    async def test_comprehensive_case_insensitive_handling(self, simple_adapter, mixed_case_status):
        ...
        with patch.object(simple_adapter, '_find_job_by_symbol_day', return_value=None) as mock_find:
...
class TestPostgresPoolRaceCondition:
    ...
    async def test_concurrent_pool_initialization(self):
        ...
        from marketpipe.ingestion.infrastructure.postgres_repository import PostgresIngestionJobRepository
