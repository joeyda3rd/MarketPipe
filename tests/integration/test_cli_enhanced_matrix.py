    ...
    def _create_factory_reset_edge_cases(self) -> List[EdgeCaseTest]:
        ...
        return [
            ...
            EdgeCaseTest(
                ...
                options={"--base-dir": "/nonexistent/path", "--dry-run": True, "--confirm-nuclear": True},
                expected_exit_code=0,
    ...
    def generate_edge_case_report(self, results: List[EdgeCaseResult]) -> str:
        ...
        if failed_results:
            ...
            for result in failed_results:
                ...
                report.append(f"Exit Code: {result.exit_code} (expected: {result.test.expected_exit_code})")
        ...
        for cmd_path in fast_commands:
            ...
            if result.execution_time_ms > MAX_FAST_TIME_MS:
                slow_commands.append(
                    f"{' '.join(cmd_path)}: {result.execution_time_ms:.1f}ms"
                )
