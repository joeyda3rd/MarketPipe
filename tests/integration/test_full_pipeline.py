...
from marketpipe.ingestion.domain.value_objects import IngestionJobId
...
def test_full_pipeline_with_validation_reports(tmp_path):
    ...
    storage_engine.write_job_bars(str(job_id), symbol.value, bars, frame="1m")
    
    # Verify data was stored
    job_bars = storage_engine.load_job_bars(str(job_id))
    assert str(job_id) in storage_engine.list_jobs()
    ...
    mock_validation_result = ValidationResult(
        ...
        total=2,
        errors=[
            BarError(ts_ns=1705312200000000000, reason="timestamp not aligned to minute boundary at index 0"),
            BarError(ts_ns=1705312260000000000, reason="extreme price movement at index 1: 1.5% change"),
        ]
    )
    validation_service._validator.validate_bars = Mock(return_value=mock_validation_result)
    
    # Trigger validation
    validation_service.handle_ingestion_completed(IngestionJobCompleted(str(job_id)))
...
def test_multiple_symbols_pipeline(tmp_path):
    ...
    def mock_validate_bars(symbol_name, bars):
        ...
        if symbol_name == "AAPL":
            return ValidationResult(symbol=symbol_name, total=2, errors=[
                BarError(ts_ns=1705312200000000000, reason="AAPL error 1")
            ])
