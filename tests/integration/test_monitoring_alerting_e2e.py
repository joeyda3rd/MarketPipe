...
class MetricsCollector:
        ...
        for rule_name, rule in self.alert_rules.items():
            if rule["metric"] == metric_name:
                ...
                if triggered:
                    ...
                    print(
                        f"🚨 ALERT: {rule_name} - {metric_name} {condition} {threshold} (actual: {value})"
                    )
    ...
    def get_alert_summary(self) -> dict:
        ...
        recent_alerts = [
            a for a in self.alerts_triggered if time.time() - a["timestamp"] < 300  # Last 5 minutes
        ]
...
class SystemHealthMonitor:
    ...
    def calculate_health_score(self) -> float:
        ...
        for _check_name, check_config in self.health_checks.items():
...
class TestMonitoringAlertingEndToEnd:
    ...
    def test_metrics_collection_pipeline(self, tmp_path):
        ...
        api_metrics = [
            {
                ...
                "value": 10,
                "labels": {"provider": "alpaca", "endpoint": "/bars"},
            },
            {
                "metric": "api_requests_failed",
                "value": 1,
                "labels": {"provider": "alpaca", "error": "timeout"},
            },
        ]
        ...
        for metric in all_metrics:
            collector.record_metric(
                metric_name=metric["metric"], value=metric["value"], labels=metric.get("labels", {})
            )
    ...
    def test_alert_triggering_and_notification(self, tmp_path):
        ...
        alert_manager.add_notification_channel(
            "email", {"type": "email", "endpoint": "ops@company.com"}
        )
        alert_manager.add_notification_channel(
            "slack", {"type": "slack", "webhook": "https://hooks.slack.com/..."}
        )
        alert_manager.add_notification_channel(
            "pager", {"type": "pagerduty", "service_key": "abc123"}
        )
        ...
        print(
            f"📊 Alert Summary: {alert_summary['total_alerts']} total, by severity: {alert_summary['by_severity']}"
        )
    ...
    def test_system_health_monitoring(self, tmp_path):
        ...
        async def run_health_monitoring():
            ...
            for cycle in range(3):
                ...
                print(
                    f"    Checks passed: {health_result['total_checks'] - health_result['critical_failures']}/{health_result['total_checks']}"
                )
    ...
    def test_dashboard_metrics_integration(self, tmp_path):
        ...
        workflow_steps = [
            # Job initialization
            {
                "metric": "ingestion_jobs_total",
                "value": 1,
                "labels": {"status": "initialized", "job_id": "dashboard-test"},
            },
            # API requests
            ...
            # Job completion
            {
                "metric": "ingestion_jobs_total",
                "value": 1,
                "labels": {"status": "completed", "job_id": "dashboard-test"},
            },
        ]

        # Record metrics with timestamps
        for _i, step in enumerate(workflow_steps):
            collector.record_metric(
                metric_name=step["metric"], value=step["value"], labels=step.get("labels", {})
            )
        ...
        dashboard_metrics["kpi"] = {
            ...
            "jobs_completed": len(
                [
                    m
                    for m in collector.metrics.get("ingestion_jobs_total", [])
        ...
        # Performance metrics
        processing_times = [
            m["value"] for m in collector.metrics.get("processing_duration_seconds", [])
        ]
        dashboard_metrics["performance"] = {
            ...
            "throughput_bars_per_second": (
                dashboard_metrics["kpi"]["total_bars_processed"] / sum(processing_times)
                if processing_times
                else 0
            ),
        }
        ...
        dashboard_metrics["resources"] = {
            "peak_memory_mb": (
                max(m["value"] for m in collector.metrics.get("memory_usage_bytes", [])) / 1024**2
                if collector.metrics.get("memory_usage_bytes")
                else 0
            ),
            "avg_cpu_percent": (
                sum(m["value"] for m in collector.metrics.get("cpu_usage_percent", []))
                / len(collector.metrics.get("cpu_usage_percent", []))
                if collector.metrics.get("cpu_usage_percent")
                else 0
            ),
        }
        ...
        print("  ⚡ Performance:")
        print(
            f"    Avg processing time: {dashboard_metrics['performance']['avg_processing_time']:.1f}s"
        )
        print(
            f"    Throughput: {dashboard_metrics['performance']['throughput_bars_per_second']:.0f} bars/sec"
        )
        ...
        for metric_name, metric_data in collector.metrics.items():
            if metric_data:
                time_series_data[metric_name] = [
                    {"timestamp": m["timestamp"], "value": m["value"]} for m in metric_data
                ]
...
def test_comprehensive_monitoring_integration_demo(tmp_path):
    ...
    alert_rules = [
        ...
        {
            "name": "high_error_rate",
            "metric": "processing_errors_total",
            "condition": "greater_than",
            "threshold": 10,
            "severity": "warning",
        },
    ...
    alert_manager.add_notification_channel(
        "slack", {"webhook": "https://hooks.slack.com/monitoring"}
    )
    ...
    async def run_comprehensive_monitoring_demo():
        ...
        print(
            f"  Total bars processed: {sum(m['value'] for m in normal_metrics if m['metric'] == 'bars_processed_total'):,}"
        )
        ...
        alert_scenarios = [
            ...
            },
            {
                "metric": "processing_errors_total",
                "value": 15,
                "description": "Processing errors spike",
            },
            {
                "metric": "processing_duration_seconds",
                "value": 45,
                "description": "Slow job processing",
            },
        ]
        ...
        for scenario in alert_scenarios:
            collector.record_metric(metric_name=scenario["metric"], value=scenario["value"])
            ...
            for alert in new_alerts:
                ...
                print(
                    f"  🚨 {scenario['description']}: Alert sent to {len(notification_result['channels'])} channels"
                )
    ...
    ), f"Monitoring system insufficient: {passed_checks}/{total_checks}"
