...
class TestSymbolsModes:
    ...
    @patch('marketpipe.ingestion.pipeline.symbol_pipeline.normalize_stage')  # Mock the SQL execution
    ...
    def test_dry_run_with_execute_precedence(self, mock_duckdb, mock_refresh, mock_scd, 
                                           mock_normalize, mock_get_provider, mock_list_providers):
    ...
    @patch('marketpipe.ingestion.pipeline.symbol_pipeline.refresh')  # Mock view refresh
    @patch('duckdb.connect')
    def test_backfill_runs_multiple_days(self, mock_duckdb, mock_refresh, mock_scd, 
                                       mock_normalize, mock_get_provider, mock_list_providers):
    ...
    def test_diff_only_path(self, mock_duckdb, mock_refresh, mock_scd, mock_list_providers):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('marketpipe.ingestion.pipeline.symbol_pipeline.fetch_providers') as mock_fetch:
                result = self.runner.invoke(root_app, [
                    "symbols", "update",
                    "-p", "dummy", 
                    "--data-dir", str(data_dir),
    ...
    @patch('marketpipe.ingestion.pipeline.symbol_pipeline.normalize_stage')  # Mock the SQL execution
    @patch('duckdb.connect')
    def test_combined_backfill_dry_run(self, mock_duckdb, mock_normalize, 
                                     mock_get_provider, mock_list_providers):
    ...
    @patch('marketpipe.ingestion.pipeline.symbol_pipeline.normalize_stage')  # Mock the SQL execution
    @patch('duckdb.connect')
    def test_performance_backfill_three_days(self, mock_duckdb, mock_normalize, 
                                           mock_get_provider, mock_list_providers):
        ...
        async def mock_fetch_symbols():
            ...
            return [Mock(meta={}, model_dump=lambda: {
                "symbol": "TEST", "name": "Test", "meta": {"provider": "dummy"}
            })]
        mock_provider.fetch_symbols = mock_fetch_symbols
        mock_get_provider.return_value = mock_provider
    ...
    @patch('marketpipe.ingestion.pipeline.symbol_pipeline.normalize_stage')  # Mock the SQL execution
    @patch('duckdb.connect')
    def test_dry_run_without_execute_shows_preview(self, mock_duckdb, mock_normalize, 
                                                  mock_get_provider, mock_list_providers):
    ...
    def test_backfill_date_after_snapshot_error(self, mock_list_providers):
        ...
        result = self.runner.invoke(root_app, [
            ...
            "-p", "dummy",
            "--backfill", "2025-06-20",
            "--snapshot-as-of", "2025-06-19",  # Earlier than backfill
            "--execute"
        ])
    ...
    def test_large_backfill_warning_and_confirmation(self, mock_confirm, mock_list_providers):
        ...
        result = self.runner.invoke(root_app, [
            "symbols", "update",
            "-p", "dummy",
            "--backfill", "2020-01-01",  # 5+ years ago
            "--snapshot-as-of", "2025-06-19",
            "--execute"
        ])
    ...
    def test_large_backfill_warning_shows_for_over_365_days(self, mock_list_providers):
        ...
        result = self.runner.invoke(root_app, [
            ...
            "-p", "dummy",
            "--backfill", "2024-01-01",
            "--snapshot-as-of", "2025-02-04",  # ~400 days
    ...
    def test_snapshot_date_format_validation(self, mock_list_providers):
        ...
        result = self.runner.invoke(root_app, [
            "symbols", "update",
            "-p", "dummy",
            "--snapshot-as-of", "invalid-date",
            "--execute"
        ])
        
        assert result.exit_code == 1
        assert "Invalid date format for --snapshot-as-of: invalid-date. Use YYYY-MM-DD." in result.output
    ...
    @patch('duckdb.connect')
    def test_progress_feedback_verbose_mode(self, mock_duckdb, mock_refresh, mock_scd, 
                                          mock_normalize, mock_get_provider, mock_list_providers):
