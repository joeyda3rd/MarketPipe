...
class FakeIngestionCheckpointRepository(IIngestionCheckpointRepository):
    ...
    async def get_all_checkpoints(
        ...
        return [
            checkpoint for (jid, symbol), checkpoint in self._checkpoints.items()
            if jid == job_id
        ]
    ...
    async def delete_checkpoints(self, job_id: IngestionJobId) -> None:
        ...
        keys_to_delete = [
            key for key in self._checkpoints.keys()
            if key[0] == job_id
        ]
    ...
    async def get_global_checkpoint(self, symbol: Symbol) -> Optional[IngestionCheckpoint]:
        ...
        symbol_checkpoints = [
            checkpoint for (job_id, sym), checkpoint in self._checkpoints.items()
            if sym == symbol
        ]
