...
class FakeMetricsCollector:
    ...
    def assert_counter_incremented(
        ...
        observations = [
            ...
            if (
                obs["name"] == name
                and obs["type"] == "counter"
                and obs["operation"] == "increment"
                and obs["labels"] == (labels or {})
            )
        ]
    ...
    def assert_histogram_observed(
        ...
        observations = [
            ...
            if (
                obs["name"] == name
                and obs["type"] == "histogram"
                and obs["operation"] == "observe"
                and obs["labels"] == (labels or {})
            )
        ]
        actual_count = len(observations)
        assert (
            actual_count >= min_observations
    ...
    def assert_gauge_set(
        ...
        if expected_value is not None:
            ...
            ), f"Expected gauge {name} = {expected_value}, got {actual_value}"

        # Also check that at least one set operation occurred
        observations = [
            obs
            for obs in self.observations
            if (obs["name"] == name and obs["type"] == "gauge" and obs["labels"] == (labels or {}))
        ]
...
def patch_prometheus_metrics(collector: FakeMetricsCollector):
    ...
    class PrometheusClientPatcher:
        ...
        def __enter__(self):
            ...
            histogram_patcher = patch(
                "prometheus_client.Histogram", side_effect=self._create_histogram
            )
...
def assert_metrics_collected(collector: FakeMetricsCollector, expected_metrics: dict[str, Any]):
    ...
    for metric_name, expected_data in expected_metrics.items():
        ...
        if metric_type == "counter":
            ...
            assert (
                actual_value >= expected_value
            ), f"Counter {metric_name} expected >= {expected_value}, got {actual_value}"
            ...
            ), f"Histogram {metric_name} expected >= {min_observations} observations, got {len(values)}"
            ...
            if "value" in expected_data:
                ...
                assert (
                    actual_value == expected_value
                ), f"Gauge {metric_name} expected {expected_value}, got {actual_value}"
