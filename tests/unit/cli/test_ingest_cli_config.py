...
"""
        ...
        try:
            with patch("marketpipe.cli._build_ingestion_services") as mock_build:
                ...
                assert "Creating ingestion job for 2 symbols" in result.stdout  # Symbol override worked
        finally:
            Path(temp_path).unlink()
    ...
    def test_direct_flags_without_config(self):
        ...
        with patch("marketpipe.cli._build_ingestion_services") as mock_build:
            ...
            # Check that the command succeeded
            assert result.exit_code == 0
            assert "Creating ingestion job for 3 symbols from 2025-02-01 to 2025-02-07" in result.stdout
    ...
    def test_missing_required_flags_error(self):
        ...
        assert "Either provide --config file OR all of --symbols, --start, and --end" in result.stdout
