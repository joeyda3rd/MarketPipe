...
def sample_df():
    ...
    return pd.DataFrame({
        "ts_ns": [1640995800000000000, 1640995860000000000],  # 2022-01-01 09:30:00, 09:31:00
        "open": [100.0, 101.0],
        "high": [101.0, 102.0], 
        "low": [99.0, 100.5],
        "close": [100.5, 101.5],
...
class TestParquetStorageEngineWrite:
    ...
    def test_write_basic(self, engine: ParquetStorageEngine, sample_df: pd.DataFrame, tmp_path: Path):
    ...
    def test_write_file_exists_without_overwrite_raises_error(self, engine: ParquetStorageEngine, sample_df: pd.DataFrame):
    ...
    def test_write_cleanup_on_failure(self, mock_write_table, engine: ParquetStorageEngine, sample_df: pd.DataFrame, tmp_path: Path):
    ...
    def test_append_to_job_existing_file(self, engine: ParquetStorageEngine, sample_df: pd.DataFrame):
...
class TestParquetStorageEngineRead:
    ...
    def test_load_partition_multiple_files(self, engine: ParquetStorageEngine, sample_df: pd.DataFrame):
        ...
        sample_df2["ts_ns"] = [1640995980000000000, 1641000000000000000]  # Different times
        engine.write(
            sample_df2,
            frame="1m",
            symbol="AAPL",
    ...
    def test_load_job_bars_single_symbol(self, engine: ParquetStorageEngine, sample_df: pd.DataFrame):
    ...
    def test_load_job_bars_multiple_symbols(self, engine: ParquetStorageEngine, sample_df: pd.DataFrame):
    ...
    def test_load_symbol_data_with_date_filter(self, engine: ParquetStorageEngine, sample_df: pd.DataFrame):
...
class TestParquetStorageEngineUtilities:
    ...
    def test_validate_integrity_healthy(self, engine: ParquetStorageEngine, sample_df: pd.DataFrame):
    ...
    def test_validate_integrity_with_corrupted_file(self, engine: ParquetStorageEngine, sample_df: pd.DataFrame, tmp_path: Path):
        ...
        corrupted_path = tmp_path / "frame=1m" / "symbol=AAPL" / "date=2022-01-02" / "corrupted.parquet"
...
class TestParquetStorageEngineErrorHandling:
    ...
    def test_load_partition_handles_read_errors(self, mock_read_table, engine: ParquetStorageEngine, sample_df: pd.DataFrame, tmp_path: Path):
    ...
    def test_load_job_bars_handles_read_errors(self, mock_read_parquet, engine: ParquetStorageEngine, tmp_path: Path):
