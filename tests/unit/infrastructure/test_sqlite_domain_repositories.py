...
class TestSqliteSymbolBarsRepository:
    ...
    @pytest.mark.asyncio
    async def test_get_nonexistent_aggregate(
    ...
    @pytest.mark.asyncio
    async def test_save_duplicate_aggregate_raises_error(
        self, temp_db_path, sample_symbol, sample_trading_date
    ):
    ...
    @pytest.mark.asyncio
    async def test_concurrency_error_on_version_mismatch(
        self, temp_db_path, sample_symbol, sample_trading_date
    ):
...
class TestSqliteOHLCVRepository:
    ...
    async def test_save_and_get_bars(self, temp_db_path, sample_bars):
        ...
        time_range = TimeRange(sample_bars[0].timestamp, sample_bars[-1].timestamp)
    ...
    async def test_get_bars_for_multiple_symbols(self, temp_db_path):
        ...
        for i in range(1, len(retrieved_bars)):
            ...
            assert prev_bar.timestamp < curr_bar.timestamp or (
                prev_bar.timestamp == curr_bar.timestamp
                and prev_bar.symbol.value <= curr_bar.symbol.value
            )
    ...
    async def test_count_bars(self, temp_db_path, sample_bars):
        ...
        time_range = TimeRange(sample_bars[0].timestamp, sample_bars[2].timestamp)
...
class TestRepositoryIdempotency:
    ...
    @pytest.mark.asyncio
    async def test_multiple_saves_are_idempotent(
        self, temp_db_path, sample_symbol, sample_trading_date
    ):
