...
class TestRateLimiterLoad:
    ...
    def test_load_2x_vendor_limit_5_minutes_sync(self):
        ...
        expected_max_requests = vendor_limit_per_min * (elapsed / 60.0) + vendor_limit_per_min  # Include burst
        assert requests_made <= expected_max_requests, f"Made {requests_made} requests, expected <= {expected_max_requests}"
        
        print(f"Load test completed: {requests_made} requests in {elapsed:.1f}s ({actual_rate:.1f} req/min)")
    ...
    async def test_load_2x_vendor_limit_5_minutes_async(self):
        ...
        print(f"Async load test completed: {requests_made} requests in {elapsed:.1f}s ({actual_rate:.1f} req/min)")
    ...
    def test_concurrent_load_mixed_sync_async(self):
        ...
        assert total_requests <= max_expected, f"Made {total_requests} requests, expected <= {max_expected}"
        
        print(f"Concurrent test: {total_requests} total requests ({sum(sync_requests)} sync, {sum(async_requests)} async)")
    ...
    def test_burst_handling_under_load(self):
        ...
        assert 1.5 <= sustained_rate <= 2.5, f"Sustained rate was {sustained_rate:.2f} req/s, expected ~2 req/s"
        
        total_elapsed = time.monotonic() - start_time
        total_requests = 10 + sustained_requests  # Burst + sustained
        
        print(f"Burst test: {total_requests} requests in {total_elapsed:.1f}s "
