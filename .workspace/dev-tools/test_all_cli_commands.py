...
class ComprehensiveCLITester:
    ...
    def run_basic_command_validation(self) -> tuple[bool, str]:
        ...
        try:
            ...
            for i, command in enumerate(commands, 1):
                print(
                    f"   Testing {i}/{len(commands)}: {' '.join(command.path)}", end="", flush=True
                )
            ...
            try:
                ...
                if execution_time > max_time_sec:
                    slow_commands.append(
                        f"{' '.join(cmd_path)}: {execution_time:.2f}s (max: {max_time_sec}s)"
                    )
                    success = False
    ...
    def run_pytest_integration(self) -> tuple[bool, str]:
        ...
        try:
            ...
            summary_line = next(
                (line for line in output_lines if "passed" in line and "=" in line), ""
            )

            report = "Pytest CLI Tests Results:\n"
    ...
    def generate_comprehensive_report(self) -> str:
        ...
        report.append(
            f"- Overall Success Rate: {successful_phases/total_phases*100:.1f}%"
            if total_phases > 0
    ...
    def run_comprehensive_testing(
        self, include_pytest: bool = True, quick_mode: bool = False
    ) -> bool:
