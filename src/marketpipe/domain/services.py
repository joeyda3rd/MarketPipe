...
class OHLCVCalculationService(DomainService):
    ...
    def daily_summary(self, bars: Iterable[OHLCVBar]) -> DailySummary:
        ...
        for bar in bars_list:
            ...
            if bar.timestamp.trading_date() != trading_date:
                raise ValueError(f"All bars must be from same trading date. Found {bar.timestamp.trading_date()}, expected {trading_date}")
    ...
    def resample(self, bars: Iterable[OHLCVBar], frame_seconds: int) -> List[OHLCVBar]:
        ...
        for bar in bars_list:
            ...
            seconds_since_midnight = (bar_timestamp.hour * 3600 + 
...
class MarketDataValidationService(DomainService):
    ...
    def validate_bar(self, bar: OHLCVBar) -> List[str]:
        ...
        if not (bar.high_price >= bar.open_price and 
            ...
            errors.append(f"High price ({bar.high_price.value}) must be >= open ({bar.open_price.value}), close ({bar.close_price.value}), and low ({bar.low_price.value})")
        
        if not (bar.low_price <= bar.open_price and
                bar.low_price <= bar.close_price):
            errors.append(f"Low price ({bar.low_price.value}) must be <= open ({bar.open_price.value}) and close ({bar.close_price.value})")
        ...
        if len(bars) > 1:
            for i in range(1, len(bars)):
                if bars[i].timestamp.value <= bars[i-1].timestamp.value:
                    all_errors.append(f"Bar {i}: Timestamps must be monotonic. "
    ...
    def _validate_trading_hours_window(
        ...
        if not (start_minutes <= bar_minutes_et <= end_minutes):
            errors.append(f"Bar timestamp appears to be outside regular trading hours "
    ...
    def _emit_validation_failed_event(self, bars: List[OHLCVBar], errors: List[str]) -> None:
