...
def run_scd_update(
    ...
    try:
        ...
        logger.info(f"Diff summary - Insert: {insert_count}, Update: {update_count}, Unchanged: {unchanged_count}")
    ...
    try:
        ...
        if insert_count > 0 or update_count > 0:
            ...
            if insert_count > 0 and update_count > 0:
                ...
                new_rows_table = db.execute(f"""
                    SELECT {column_select}
                    FROM diff_insert
                    UNION ALL
                    SELECT {column_select.replace('$1', '$2')}
                    FROM diff_update
                """, [snapshot_date, snapshot_date]).arrow()
            elif insert_count > 0:
                # Only inserts
                new_rows_table = db.execute(f"""
                    SELECT {column_select}
        ...
        if dry_run:
            ...
            logger.info(f"[DRY RUN] Would write {total_rows} total rows ({total_new_rows} new) to {data_dir}")
            return stats
    ...
    if len(all_data_to_write) > 0:
        try:
            ...
            if 'valid_to' in combined_df.columns:
                ...
                combined_df['valid_to'] = pd.to_datetime(combined_df['valid_to'], errors='coerce').dt.date
            ...
            logger.info(f"Successfully wrote {total_rows} total rows ({total_new_rows} new) to partitioned dataset at {data_dir}")
