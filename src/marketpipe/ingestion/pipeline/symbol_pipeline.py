...
from marketpipe.metrics import SYMBOLS_NULL_RATIO, SYMBOLS_SNAPSHOT_RECORDS
...
async def fetch_providers(names: Sequence[str], snapshot_as_of: _dt.date) -> list:
...
def records_to_stage(conn: duckdb.DuckDBPyConnection, records: list) -> None:
    """
...
def _update_null_ratio_metrics(con: duckdb.DuckDBPyConnection) -> None:
    ...
    try:
        ...
        cols = [c[0] for c in columns_result if c[0] not in ("id", "valid_from", "valid_to")]
        ...
        for col in cols:
            try:
                null_result = con.sql(
                    f"SELECT COUNT(*) FROM v_symbol_latest WHERE {col} IS NULL"
                ).fetchone()
