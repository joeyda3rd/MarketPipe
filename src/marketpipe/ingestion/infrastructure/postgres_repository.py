...
from .repositories import REPO_QUERIES, REPO_LATENCY
...
class PostgresIngestionJobRepository(IIngestionJobRepository):
            ...
            try:
                ...
                async with pool.acquire() as conn:
                    result = await conn.execute(
                        "DELETE FROM ingestion_jobs WHERE payload->>'job_id' = $1",
                        str(job_id)
                    )
    ...
    def _serialize_job_to_dict(self, job: IngestionJob) -> Dict[str, Any]:
        ...
        return {
            ...
            "completed_partitions": [self._serialize_partition(p) for p in job.completed_partitions],
        }
    ...
    def _deserialize_job_from_payload(self, payload: str) -> IngestionJob:
        ...
        created_at = datetime.fromisoformat(job_dict["created_at"]) if job_dict.get("created_at") else datetime.now()
        ...
        failed_at = datetime.fromisoformat(job_dict["failed_at"]) if job_dict.get("failed_at") else None
