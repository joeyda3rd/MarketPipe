...
def create_ingestion_job_repository_with_url(database_url: Optional[str] = None) -> IIngestionJobRepository:
    """
    ...
    """
    if database_url and database_url.split("://", 1)[0].startswith("postgres"):
        logger.info(f"Using PostgreSQL repository with explicit DSN: {_mask_credentials(database_url)}")
        return PostgresIngestionJobRepository(database_url)
    else:
