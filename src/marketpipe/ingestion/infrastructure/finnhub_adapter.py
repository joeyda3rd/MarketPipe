...
class FinnhubMarketDataAdapter(IMarketDataProvider):
    ...
    def __init__(
        ...
        self.log.info(
            f"Finnhub adapter initialized with {rate_limit_per_minute} requests/min limit"
        )
    ...
    async def fetch_bars_for_symbol(
        ...
        try:
            ...
            if response_data.get("s") == "ok" and "c" in response_data:
                ...
                self.log.warning(
                    f"No data available for {symbol.value} in the specified time range"
                )
                return []
            else:
    ...
    def _parse_finnhub_response(
        ...
        try:
            ...
            min_length = min(
                len(closes), len(highs), len(lows), len(opens), len(timestamps), len(volumes)
            )
            ...
            for i in range(min_length):
                try:
                    ...
                    self.log.warning(
                        f"Failed to parse bar data for {symbol.value} at index {i}: {e}"
                    )
                    continue
