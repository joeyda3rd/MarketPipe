...
class SimpleJobRepository:
    ...
    async def upsert(self, symbol: str, day: str, status: str) -> None:
        ...
        if status not in _SIMPLE_TO_STATE:
            raise ValueError(f"Invalid status '{status}'. Must be one of: {list(_SIMPLE_TO_STATE.keys())}")
    ...
    async def mark_done(self, symbol: str, day: str, status: str = "done") -> None:
        ...
        if status not in ["done", "error", "cancelled"]:
            raise ValueError(f"Invalid final status '{status}'. Must be one of: done, error, cancelled")
    ...
    async def _find_job_by_symbol_day(self, symbol: Symbol, day: date) -> Optional[IngestionJob]:
        ...
        for job in jobs:
            if (job.symbols and symbol in job.symbols and 
                self._extract_day_from_job(job) == day):
                return job
        
        return None

    def _create_minimal_job(self, symbol: Symbol, day: date, state: ProcessingState) -> IngestionJob:
        ...
        time_range = TimeRange(
            start=Timestamp(day_start),
            end=Timestamp(day_end)
        )
    ...
    def _update_job_state(self, job: IngestionJob, new_state: ProcessingState) -> None:
        ...
        if new_state == ProcessingState.IN_PROGRESS and not job.started_at:
            job._started_at = now
        elif new_state in [ProcessingState.COMPLETED, ProcessingState.FAILED, ProcessingState.CANCELLED]:
