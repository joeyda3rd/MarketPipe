...
def factory_reset(
    confirm_nuclear: bool = typer.Option(
        False, "--confirm-nuclear", help="Confirm you want to perform nuclear data wipe"
    ),
    keep_schema: bool = typer.Option(
        False, "--keep-schema", help="Keep database schema files and README files"
    ),
    backup_before_wipe: bool = typer.Option(
        False, "--backup-before-wipe", help="Create backup archive before wiping"
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", "-n", help="Show what would be deleted without making changes"
    ),
    base_directory: Path = typer.Option(
        Path.cwd(), "--base-dir", help="Base directory to search for data files"
    ),
):
    """
    ...
    confirmation = typer.prompt("\nType 'FACTORY-RESET-CONFIRMED' to proceed", type=str)
