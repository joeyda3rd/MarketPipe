...
def prune_parquet(
    ...
    parquet_root: Path = typer.Option(
        Path("data/parquet"), "--root", help="Root directory containing parquet files"
    ),
    ...
    try:
        ...
        for file in parquet_root.rglob("*.parquet"):
            try:
                        ...
                        try:
                            ...
                            for i, p in enumerate(path_parts[:-2]):
                                if re.match(r"\d{4}$", p):  # Year
                                    ...
                                    if re.match(r"\d{1,2}$", month_part) and re.match(
                                        r"\d{1,2}", day_part.split(".")[0]
                                    ):
                ...
                if date_found < cutoff:
                    ...
                    if dry_run:
                        typer.echo(
                            f"[DRY RUN] Would delete: {file} ({humanize.naturalsize(file_size)}) - {date_found}"
                        )
                    else:
                        typer.echo(
                            f"Deleting: {file} ({humanize.naturalsize(file_size)}) - {date_found}"
                        )
                        ...
                        try:
                            ...
                            record_metric(
                                "data_pruned_bytes", file_size, source="prune", provider="parquet"
                            )
                        except ImportError:
                        ...
                        try:
                            ...
                            event = DataPruned(data_type="parquet", amount=file_size, cutoff=cutoff)
        ...
        if dry_run:
            ...
            typer.echo(
                f"🗑️ Would remove files totaling: {humanize.naturalsize(bytes_pruned) if bytes_pruned else '0 bytes'}"
            )
        else:
            if bytes_pruned > 0:
                typer.secho(
                    f"\n✅ Removed {humanize.naturalsize(bytes_pruned)} from {files_found} files older than {cutoff}",
                    fg="green",
                )
            else:
...
def prune_database(
    ...
    try:
        ...
        if not hasattr(repo, "count_old_jobs") or not hasattr(repo, "delete_old_jobs"):
            typer.secho(
                f"⚠️ {backend_type} backend does not support pruning operations.", fg="yellow"
            )
            ...
            try:
                ...
                if deleted > 0:
                    ...
                    try:
                        ...
                        record_metric(
                            "data_pruned_rows", deleted, source="prune", provider=metric_type
                        )
                    except ImportError:
                        pass  # Metrics not available
                    ...
                    try:
                        ...
                        event = DataPruned(
                            data_type=backend_type.lower(), amount=deleted, cutoff=cutoff
                        )
