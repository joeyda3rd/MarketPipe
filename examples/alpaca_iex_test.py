...
async def test_iex_endpoints():
    ...
    iex_endpoints = [
        ...
        (
            "IEX Latest Quote",
            "https://data.alpaca.markets/v1beta1/stocks/AAPL/quotes/latest",
            {"feed": "iex"},
        ),
        (
            "IEX Latest Trade",
            "https://data.alpaca.markets/v1beta1/stocks/AAPL/trades/latest",
            {"feed": "iex"},
        ),
        # IEX Historical data - v1beta1
        (
            "IEX Historical Quotes",
            "https://data.alpaca.markets/v1beta1/stocks/AAPL/quotes",
            {
        ...
        (
            "IEX Historical Trades",
            "https://data.alpaca.markets/v1beta1/stocks/AAPL/trades",
            {
                "start": "2025-01-02T00:00:00Z",
                "end": "2025-01-02T23:59:59Z",
                "feed": "iex",
        ...
        (
            "IEX Latest Bar",
            "https://data.alpaca.markets/v1beta1/stocks/AAPL/bars/latest",
            {"feed": "iex"},
        ),
        (
            "IEX Historical Bars",
            "https://data.alpaca.markets/v1beta1/stocks/AAPL/bars",
            {
                "start": "2025-01-02T00:00:00Z",
                "end": "2025-01-02T23:59:59Z",
                "feed": "iex",
        ...
        (
            "IEX Alt Path",
            "https://data.alpaca.markets/v1/stocks/quotes/latest",
            ...
            try:
                ...
                if status_code == 200:
                            ...
                            if len(str(data)) < 500:
                                ...
                                for key, value in list(data.items())[:3]:
                                    if isinstance(value, list) and value:
                                        print(
                                            f"   📊 {key}: list with {len(value)} items, sample: {value[0] if value else 'empty'}"
                                        )
                                    else:
                                        print(
                                            f"   📊 {key}: {type(value)} = {str(value)[:50]}{'...' if len(str(value)) > 50 else ''}"
                                        )
                        else:
                    ...
                    if "forbidden" in content.lower():
                        ...
                        print("   ❌ 403 HTML Error (likely subscription required)")
