...
def validate_naming_convention(env_var: str, provider_name: str) -> List[str]:
    ...
    if not env_var.startswith("MP_"):
        errors.append(f"Should start with 'MP_' prefix")
    ...
    if not re.match(r'^[A-Z0-9_]+$', env_var):
        errors.append(f"Should only contain uppercase letters, numbers, and underscores")
...
def main() -> int:
    ...
    print(f"Found {len(all_required_vars)} required variables across {len(settings_env_vars)} providers")
    ...
    allowed_system_vars = {
        'DATABASE_URL', 'MP_DB', 'METRICS_DB_PATH', 'MP_DATA_DIR',
        'PROMETHEUS_PORT', 'METRICS_ENABLED', 'LOG_LEVEL', 'MASK_SECRETS',
        'TESTING_MODE'
    }
    ...
    for provider_name, provider_vars in settings_env_vars.items():
        for env_var in provider_vars:
            ...
            if naming_errors:
                errors.append(f"Naming convention violation for {env_var} (provider: {provider_name}):")
            ...
            if line.startswith('#') and ('PROVIDER' in line.upper() or 'TARGET' in line.upper()):
                ...
                if current_section and section_vars:
                    ...
                    if section_vars != sorted_vars:
                        warnings.append(f"Variables not alphabetically sorted in section '{current_section}':")
