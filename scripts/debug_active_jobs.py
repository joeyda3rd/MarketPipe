...
def check_active_jobs():
    ...
    try:
            ...
            if 'payload' in row_dict and row_dict['payload']:
                try:
                    ...
                    if payload.get('state') in ['pending', 'PENDING', 'in_progress', 'IN_PROGRESS']:
                        active_count += 1
                        print(f"   🔴 ACTIVE JOB - may cause conflicts!")
                    else:
                        print(f"   ✅ Completed/inactive")
        ...
        if active_count > 0:
            ...
            print(f"💡 To clear them, run:")
            print(f"   python scripts/debug_active_jobs.py --clear")
        else:
...
def clear_active_jobs():
    ...
    try:
        ...
        print(f"💡 Scheduling conflicts should now be resolved")
