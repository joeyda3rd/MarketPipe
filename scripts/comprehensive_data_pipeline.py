...
class MarketPipelineRunner:
    ...
    def __init__(self, dry_run: bool = False, provider: str = "fake", live_alpaca: bool = False):
        ...
        self.log_and_print(f"🚀 MarketPipe Comprehensive Pipeline")
        ...
        if live_alpaca:
            self.log_and_print(f"🔴 LIVE ALPACA MODE: Will use real API credentials")
    ...
    def generate_summary(self):
        ...
        self.log_and_print(f"📊 Pipeline Execution Summary:")
        ...
        if self.live_alpaca:
            self.log_and_print(f"   • Live Alpaca Data: ✅")
        ...
        self.log_and_print(f"💡 Next steps:")
        self.log_and_print(f"   • Start metrics server: python -m marketpipe metrics --port 8000")
        self.log_and_print(f"   • Query data: python -m marketpipe query \"SELECT symbol, COUNT(*) FROM 'data/raw/**/*.parquet' WHERE symbol IS NOT NULL GROUP BY symbol\"")
        self.log_and_print(f"   • Monitor validation: python -m marketpipe validate-ohlcv --list")
